  ...
  async init() {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        // Kamu bisa format ulang tanggal dan waktu jika perlu
        const formatted = data.map((item) => {
          const tanggalObj = new Date(item.timestamp);
          return {
            ...item,
            tanggal: tanggalObj.toLocaleDateString('id-ID'),
            waktu: tanggalObj.toLocaleTimeString('id-ID'),
            kamera: item.kamera || 'Tidak diketahui',
            aktivitas: item.aktivitas || 'Tidak diketahui',
            alarm: item.alarm || 'nonaktif',
          };
        });

        this.page.updateRiwayatTable(formatted);
      } else {
        this.page.showEmpty();
      }
    } catch (error) {
